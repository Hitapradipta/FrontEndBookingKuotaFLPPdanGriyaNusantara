    ...
    <div class="container mx-auto p-4">
        <!-- Header Informasi Kuota Induk GN -->
        ...
        <VCardTitle class="text-h6 font-weight-bold">
            ...
            >
            Balik ke tabel utama [Kuota GN {{ currentYear }}]
            </VBtn>
        </VCardTitle>
        <VCardText>
            ...
            <thead>
                <tr>
                <th class="text-left bg-green-600 text-white font-semibold">Kuota Induk GN Tahun</th>
                <th class="text-right bg-green-600 text-white font-semibold">Total Kuota</th>
                <th class="text-right bg-green-600 text-white font-semibold">Kuota Belum Terlokasi</th>
                <th class="text-right bg-green-600 text-white font-semibold">Sisa Kuota</th>
                <th class="text-right bg-green-600 text-white font-semibold">Persentase Utilisasi</th>
                <th class="text-center bg-green-600 text-white font-semibold">Activated</th>
                </tr>
            </thead>
            <tbody>
                <tr v-if="loading">
                <td colspan="6" class="py-6 text-center text-gray-500">Memuat data kuota induk GN...</td>
                </tr>
                <tr v-else>
        ...
        <!-- Tombol Aksi -->
        <div class="flex justify-between items-center mb-4 bg-green-600 text-white p-3 rounded-lg shadow-md">
        <div>
        ...
        <VTable class="submission-table">
            ...
            <tr class="bg-gray-800 text-white">
                ...
                <th class="text-right p-4">SISA KUOTA GN</th>
...
import {
    ...
    // Data mock untuk Griya Nusantara
    const mockMitraData = ref([
    {
        id: 1,
        tahun: 2024,
        namaMitra: 'Bank Hijau Nusantara',
        kuota: 2000000000n,
        kuotaTerpakai: 400000000n,
        sisaKuota: 1600000000n,
        utilisasi: 20.0,
    },
    {
        id: 2,
        tahun: 2024,
        namaMitra: 'Koperasi GN Sejahtera',
        kuota: 1500000000n,
        kuotaTerpakai: 300000000n,
        sisaKuota: 1200000000n,
        utilisasi: 20.0,
    },
    ]);
    ...
    const formTitle = computed(() => (editedIndex.value === -1 ? 'Tambah Mitra GN Baru' : 'Edit Mitra GN'));
    ...
    function goBack() {
    router.push({ name: 'grinus-quota-grinus-kuota' }); // Ganti route sesuai kebutuhan
    }
    ...
    // --- Fetch Data dari API (untuk GN) ---
    async function fetchKuotaInduk() {
    loading.value = true;
    try {
        const response = await fetch(`${API_BASE}/kuota-gn?tahun=${currentYear}`);
        if (!response.ok) throw new Error('Gagal memuat data kuota induk GN');
        ...
        console.error('Error fetching kuota induk GN:', error);
        // Fallback data
        kuotaInduk.value.totalKuota = 50000000000n; // 50 miliar (contoh)
        kuotaInduk.value.kuotaBelumTerlokasi = 50000000000n;
        kuotaInduk.value.sisaKuota = 50000000000n;
    ...
    onMounted(() => {
    fetchKuotaInduk();
    console.log('User ID untuk GN:', userId.value);
    });
    </script>
